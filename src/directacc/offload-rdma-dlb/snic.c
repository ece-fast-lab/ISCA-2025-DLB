...
static void *snic_dlb_representative(void *__args)
{
    ...
    for (int j = 0; j < meta_size; j++) {
        // ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[j].qid = 31 - (num_dlb_pp-1-tid)%8; //31 - (lrand48() % num_dlb_pp)
        ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[j].qid = (num_dlb_pp > 8) ?  31 - (lrand48() % 8) : 31 - (num_dlb_pp-1-tid)%8;; //31 - (lrand48() % num_dlb_pp)
    ...
    while (test_finished == 0) {
        ...
        // printf("here %d\n", ecnt);
        // post_recv(&conn_ctxs[meta_conn_id], 4*sizeof(struct rdma_metadata));
        ...
        int valid_qe = 0;
        while ((valid_qe == 0) && test_finished == 0) {
            if (metadata_offset + 4 > meta_size && metadata_offset + 1 < meta_size) {
                for (int j = valid_qe; j < (meta_size-metadata_offset); j++) {
                    if (((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset+j].valid != 0) {
                        ...
                        // ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx+j].qid = 31 - (lrand48() % num_dlb_pp);
                        // ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx+j].qid = 31 - ((buf_idx+j) % num_dlb_pp);
                        ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx+j].cmd_byte = 8;

                        ((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset+j].valid = 0;
                        valid_qe++;
                    } else {
                ...
                // if (((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset].valid != 0) {
                //     ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx].data = ((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset].addr;
                //     // ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx].qid = 31 - (lrand48() % num_dlb_pp);
                //     ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx].cmd_byte = 8;
                //     ((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset].valid = 0;
                //     // metadata_offset = (metadata_offset + 1) % meta_size;
                //     valid_qe++;
                // } else {
                //     ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx].cmd_byte = 0;
                // }

                for (int j = valid_qe; j < 4; j++) {
                    if (((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset+j].valid != 0) {
                        ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx+j].data = ((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset+j].addr;
                        // ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx+j].qid = 31 - (lrand48() % num_dlb_pp);
                        // ((dlb_enqueue_qe_t *)(conn_ctxs[dlb_conn_id].data_buf))[buf_idx+j].qid = 31 - ((buf_idx+j) % num_dlb_pp);
        ...
        // printf("valid qe is %d\n", valid_qe);

        buf_idx += valid_qe;
        local_credits[tid] -= valid_qe;

        if (ecnt != 0 && ecnt % (meta_size) == 0 && tid != num_client_conns-1) {
            // memcpy(conn_ctxs[meta_conn_id].data_buf+metadata_mr_size, READY_MSG, sizeof(READY_MSG));
        ...
        // if (ecnt != 0 && ecnt % 1000 == 0) printf("T%d metadata_offset is %d, ecnt = %d, valid_qe=%d, global_credits=%d\n", tid, metadata_offset, ecnt, valid_qe, global_credits);
        // if (tid == 0) printf("T%d metadata_offset is %d, ecnt = %d, valid_qe=%d, global_credits=%d\n", tid, metadata_offset, ecnt, valid_qe, global_credits);
        // if (valid_qe != 4)
        // if (tid == num_client_conns-1) printf("T%d metadata_offset is %d, ecnt = %d, valid_qe=%d, global_credits=%d\n", tid, metadata_offset, ecnt, valid_qe, global_credits);
        if (tid == num_client_conns-1 && ecnt == num_events) test_finished = 1;
    }    
}
...
static void *credit_manager(void *__args) {
    ...
    while (!test_finished) {
        // if (snic_global_credits < 1024) {
            global_credits = query_credits(&dev_ctx, &conn_ctxs[0], sizeof(uint32_t));
            while (global_credits < (uint32_t)128) {
            ...
            // printf("new_credits is %d\n", global_credits);
            // atomic_put(new_credits);
            // printf("snic credit is %d\n", atomic_load(&snic_global_credits));
            // snic_global_credits += acquire_credits(&dev_ctx, &(conn_ctxs[0]), sizeof(uint32_t), global_credits_64, 0);
        // }
    ...
    // global_credits = query_credits(&dev_ctx, &conn_ctxs[0], sizeof(uint32_t));
    // global_credits_64 = *(uint64_t *)(conn_ctxs[0].data_buf);
    // // local_credits = acquire_credits(&dev_ctx, &conn_ctxs[0], sizeof(uint32_t), global_credits_64, (global_credits_64 - (uint64_t)credit_batch));
    ...
    //                 global_credits = query_credits(&dev_ctx, &conn_ctxs[0], sizeof(uint32_t));
    //             }
    //             while (local_credits[i] < (uint32_t)4) {
    //                 global_credits = query_credits(&dev_ctx, &conn_ctxs[0], sizeof(uint32_t));
    //                 global_credits_64 = *(uint64_t *)(conn_ctxs[0].data_buf);
    //                 local_credits[i] += acquire_credits(&dev_ctx, &(conn_ctxs[0]), sizeof(uint32_t), global_credits_64, (global_credits_64 - (uint64_t)credit_batch));
    //                 // global_credits_64 = *(uint64_t *)(conn_ctxs[0].data_buf + 8);
    //                 // global_credits = query_credits(&dev_ctx, &conn_ctxs[0], sizeof(uint32_t));
    //                 // printf("new global credits is %d, local credits is %d\n", *(uint32_t *)(conn_ctxs[0].data_buf), local_credits);
    //             }
    //             // printf("local_credits[%d]=%d\n", i, local_credits[i]);
    //         }
    //     }
    // }
