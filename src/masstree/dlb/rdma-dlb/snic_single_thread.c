...
int main (int argc, char *argv[]) {
    ...
    while (ecnt < num_events) {
        ...
        #if DEBUG
            printf("local credits is %d\n", local_credits);
            printf("local address is %p\n", conn_ctxs[2].local_mr.addr);
        #endif
        ...
        while (valid_qe != 4) {
            ...
            printf("wr_id 0 is %d\n", ((struct rdma_metadata *)(conn_ctxs[2].data_buf))[metadata_offset].wr_id);
            printf("size 0 is %d\n", ((struct rdma_metadata *)(conn_ctxs[2].data_buf))[metadata_offset].size);
            printf("addr 0 is %d\n", ((struct rdma_metadata *)(conn_ctxs[2].data_buf))[metadata_offset].addr);
            ...
            for (int j = 0; j < 4; j++) {
                if (((struct rdma_metadata *)(conn_ctxs[2].data_buf))[metadata_offset+j].valid != 0) {
                    ...
                    ((dlb_enqueue_qe_t *)(conn_ctxs[0].data_buf))[buf_idx+j].opaque = ((struct rdma_metadata *)(conn_ctxs[2].data_buf))[metadata_offset+j].wr_id;
                    if (((dlb_enqueue_qe_t *)(conn_ctxs[0].data_buf))[buf_idx+j].opaque<0 || ((dlb_enqueue_qe_t *)(conn_ctxs[0].data_buf))[buf_idx+j].opaque>=200) printf("opaque is %d\n", ((dlb_enqueue_qe_t *)(conn_ctxs[0].data_buf))[buf_idx+j].opaque);
