 ...
 */
#ifndef _CClientRDMAConnection_
#define _CClientRDMAConnection_
#include <arpa/inet.h>
...
int ClientRDMAConnection::pp_get_port_info(struct ibv_context *context, int port,
		     struct ibv_port_attr *attr)
{
	return ibv_query_port(context, port, attr);
}

void ClientRDMAConnection::wire_gid_to_gid(const char *wgid, union ibv_gid *gid)
{
	char tmp[9];
	__be32 v32;
	int i;
...
void ClientRDMAConnection::gid_to_wire_gid(const union ibv_gid *gid, char wgid[])
{
	uint32_t tmp_gid[4];
	int i;
...
struct pingpong_context* ClientRDMAConnection::pp_init_ctx(struct ibv_device *ib_dev, int rx_depth, int port, int use_event, int id)
{
...
int ClientRDMAConnection::pp_post_recv(struct pingpong_context *ctx, int wr_id, bool sync)
{
	struct ibv_sge list;
...
int ClientRDMAConnection::pp_close_ctx(struct pingpong_context *ctx)
{
...
struct pingpong_dest* ClientRDMAConnection::pp_client_exch_dest(char *servername, int server_qpn0,/*int port,*/ const struct pingpong_dest *my_dest)
{
...
int ClientRDMAConnection::pp_connect_ctx(struct pingpong_context *ctx, int port, int my_psn, int sl, struct pingpong_dest *dest, int sgid_idx)
{
...
int ClientRDMAConnection::pp_post_send(struct pingpong_context *ctx, uint32_t qpn, unsigned int length, int wr_id)
{
	struct ibv_sge list;
...
ClientRDMAConnection::ClientRDMAConnection(int id, int isLast, char *ib_devname_in, int gidx_in, int remote_qp0_in, char* servername)
{
	...
	if(id == 0) {
		...
		printf("  remote address: LID 0x%04x, QPN 0x%d, PSN 0x%06x, GID %s\n",rem_dest->lid, rem_dest->qpn, rem_dest->psn, gid);
	}
