...
int main(int argc, char **argv)
{
    ...
    tx_args.port = dlb_attach_dir_port(domain, tx_port_id, NULL);
    ...
    rx_args.port = dlb_attach_dir_port(domain, rx_port_id, NULL);
    ...
    for (i = 0; i < num_workers; i++) {
        ...
        worker_args[i].port = dlb_attach_dir_port(domain, port_id, NULL);
