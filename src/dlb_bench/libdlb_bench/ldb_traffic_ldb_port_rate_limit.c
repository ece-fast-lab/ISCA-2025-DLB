...
static void *rx_traffic(void *__args)
{
    ...
    uint64_t local_end_time = __rdtsc();
...
static void *worker_fn(void *__args)
{
    ...
    double worker_latency = (double)(__rdtsc() - local_start_time) / (double)freq * 1E6;
    double worker_latency_packet = 0;
    for (i = 0; i < 8; i++) {
        for (j = 0; j < 8; j++) {
            worker_latency_packet += (double)priority_latency[args->index][i][j] / (double)freq * 1E6;
        }
    }
    worker_pkt_count[args->index] = total;
...
int main(int argc, char **argv)
{
    ...
    domain_id = create_sched_domain(dlb, num_rxs + num_workers + num_txs, 0);
    if (domain_id == -1)
    ...
    for (int i = 0; i < num_txs; i++) {
        ...
        tx_argses[i].port = dlb_attach_ldb_port(domain, tx_port_id, NULL);
    ...
    for (i = 0; i < num_workers; i++) {
        ...
        for (j = 0; j < 8; j++) {
            for (int k = 0; k < 8; k++) {
                queue_prio_lat[j] += (double)priority_latency[i][j][k] / (double)freq * 1E6;
                packet_prio_lat[k] += (double)priority_latency[i][j][k] / (double)freq * 1E6;
                worker_pkt_lat[i] += (double)priority_latency[i][j][k] / (double)freq * 1E6;
