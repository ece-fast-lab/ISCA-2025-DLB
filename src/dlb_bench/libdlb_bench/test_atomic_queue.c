...
static void *tx_traffic_atomic(void *__args)
{
    thread_args_t *args = (thread_args_t *) __args;
    dlb_event_t events[enqueue_depth];
    int64_t num_loops, i, num_tx = 0;
    int ret;
    uint64_t flow_id_start, flow_id_count;
    flow_id_start = args->flow_id_start;
    flow_id_count = args->num_flows_local;
    ...
    for (i = 0; i < enqueue_depth; i++) {
	    ...
	    // if (!sched_type)
		//     events[i].send.flow_id = args->core_mask;
    ...
    for (i = 0; (num_tx < num_events / num_txs) || (num_loops == -1); i++) {
        ...
        for (j = 0; j < enqueue_depth; j++) {
            events[j].send.priority = tx_prio;
            ...
            // events[j].adv_send.udata16 = ((num_tx + j) / num_flows == 4) ? UINT16_MAX : 0;

		    // events[j].adv_send.flow_id = rand() % flow_id_count + 1;
		    events[j].adv_send.flow_id = flow_id_start + hash(num_tx + j) % flow_id_count + 1;
		    // events[j].adv_send.rsvd2 = ((num_tx + j) / num_flows >= 4) ? 8 : 0;
        }
        ...
        for (j = 0; j < num; j++) {
            if (events[j].recv.sched_type == SCHED_UNORDERED) {
                ...
                // }
                    ...
                    (events[j].recv.sched_type == SCHED_ATOMIC && events[j].recv.flow_id != 0xABCD) ||
                    events[j].recv.error) {
...
static void *worker_fn(void *__args)
{
    ...
    for (i = 0; !worker_done; i++) {
        ...
        for (j = 0; j < num_rx; j++) {
            ...
            // }
            if (!sched_type) {
                worker_flows_served[args->index][events[j].recv.flow_id] += 1;
		        // printf("[Worker %d] event flow id is %d\n", args->index, events[j].recv.flow_id);
            }

            if (total != 0 && (total + j - num_rx) % 1000 == 0) {
                lat_arr[latency_idx] = __rdtsc() - events[j].recv.udata64;
        ...
        if (num_stages == 1) {
            // for (j = 0; j < num_rx; j++) {
            //     if (events[j].recv.udata16 != UINT16_MAX) {
            //         events[j].adv_send.flow_id = 1;
            //         // if (dlb_pop_cq(args->port, 1) != 1) {
            //         //     printf("[%s()] FAILED: Return Tokens of all %d events (iter %d)!\n",
            //         //         __func__, 1, i);
            //         //     exit(-1);
            //         // }
            //     } else {
            //         if (dlb_release(args->port, 1) != 1) {
            //             printf("[%s()] FAILED: Release of all %d events (iter %d)!\n",
            //                 __func__, 1, i);
            //             exit(-1);
            //         }
            //     }
    ...
    for (int i = 0; i < 10000; i++) {
        c2c_latency[i] = (double) lat_arr[i] / freq * 1000000.0;
...
int main(int argc, char **argv)
{
    ...
    // uint64_t *flow_patterns = NULL;
    // if (sched_type == 0) {
    //     flow_patterns = (uint64_t *)calloc(num_events, sizeof(uint64_t));
    //     if (!flow_patterns) {
    //         free(worker_threads);
    //         free(tx_threads);
    //         return -ENOMEM;
    ...
    for (int i = 0; i < num_txs; i++) {
        ...
        tx_argses[i].flow_id_start = i * (num_flows / num_txs);
        tx_argses[i].num_flows_local = num_flows / num_txs;
        tx_argses[i].index = i;
    ...
    for (i = 0; i < num_txs; i++) {
        pthread_create(&tx_threads[i], NULL, tx_traffic_atomic, &tx_argses[i]);
    }
    ...
    for (i = 0; i < num_workers; i++) {
        ...
        for (j = 0; j < 8; j++) {
            for (int k = 0; k < 8; k++) {
                queue_prio_lat[j] += priority_latency[i][j][k];
                packet_prio_lat[k] += priority_latency[i][j][k];
                worker_pkt_lat[i] += priority_latency[i][j][k];
                if (priority_latency[i][j][k] != 0) {
                    printf("Worker %d, Queue priority %d, packet priority %d has %ld cycles\n", i, j, k, priority_latency[i][j][k]);
                }
            }
        }
    }
    ...
    for (i = 0; i < 8; i++) {
        printf("Queue priority %d has %f us latency\n", i, queue_prio_lat[i] / (double)freq * 1E6 / ((double)num_events / (double)num_txs));
        total_latency += queue_prio_lat[i] / (double)freq * 1E6;
    }
    for (i = 0; i < 8; i++) {
        printf("Packet priority %d has %ld counts, %f us latency\n", i, packet_prio_count[i], packet_prio_lat[i] / (double)freq * 1E6 / (double)packet_prio_count[i]);
    }
    for (i = 0; i < 8; i++) {
        printf("Worker %d has %ld counts, %f us latency\n", i, worker_pkt_count[i], worker_pkt_lat[i] / (double)freq * 1E6 / (double)worker_pkt_count[i]);
        total_recv_counts += worker_pkt_count[i];
    }
    for (i = 0; i < 8; i++) {
        uint32_t flow_count = 0;
        for (j = 0; j < 2048; j++) {
            flow_count += (worker_flows_served[i][j] > 0)? 1 : 0;
        }
        printf("Worker %d has served %d flows\n", i, flow_count);
    }
    ...
    // if (flow_patterns != NULL) free(flow_patterns);
