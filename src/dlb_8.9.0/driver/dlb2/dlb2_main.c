			 ...
			 const struct file_operations *fops)
{
	...
	dlb2->dev_number = MKDEV(MAJOR(dlb2_devt), MINOR(dlb2_devt) +
				 dlb2->id * DLB2_NUM_FUNCS_PER_DEVICE);
	...
	if (dlb2->type == DLB2_PF || dlb2->type == DLB2_5_PF) {
		/* create a set of cdevs for PF and container usages */
		ret = cdev_add(&dlb2->cdev, dlb2->cdev.dev, DLB2_NUM_FUNCS_PER_DEVICE);
	} else {
		/*  Only one cdev is needed for a vf or a vdev  */
...
{
	dlb2->sdev[sdev_id] = device_create(dlb2_class,
				      NULL,
				      dlb2->dev_number + sdev_id + 1,
				      dlb2->sdev_dlb2[sdev_id],
				      "dlb1%02d%02d",
				      dlb2->id, sdev_id);

	if (IS_ERR(dlb2->sdev[sdev_id])) {
		dev_err(dlb2->dev,
			"%s: device_create() returned %ld, %d\n",
                                dlb2_driver_name, PTR_ERR(dlb2->sdev[sdev_id]), sdev_id);

		return PTR_ERR(dlb2->sdev[sdev_id]);
	}
...
void dlb2_sdev_destroy(struct dlb2 *dlb2)
{
	...
	for (i = 0; i < DLB2_MAX_NUM_VDEVS; i++) {
		if (dlb2->sdev[i]) {
			device_destroy(dlb2_class, dlb2->dev_number + i + 1);
			dlb2->sdev[i] = NULL;
		}
	}
}
...
struct dlb2 *dlb2_init_sdev(struct dlb2 *dlb2, int sdev_id)
{
	...
	sdev_dlb2 = devm_kzalloc(&dlb2->pdev->dev, sizeof(*sdev_dlb2), GFP_KERNEL);
	if (!sdev_dlb2)
		return NULL;

	sdev_dlb2->dev_number = dlb2->dev_number + sdev_id + 1;
	sdev_dlb2->ops = &dlb2_sdev_ops;
	sdev_dlb2->hw_ver = dlb2->hw_ver;
	sdev_dlb2->pdev = dlb2->pdev;
	sdev_dlb2->id = dlb2->id;
	sdev_dlb2->parent_dlb2 = dlb2;
	sdev_dlb2->hw.func_phys_addr = dlb2->hw.func_phys_addr;

	dlb2->sdev_dlb2[sdev_id] = sdev_dlb2;
...
static int dlb2_open(struct inode *i, struct file *f)
{
	...
	if (dlb2->dev_number == i->i_rdev) {
		/* For PF or VF/vdev */
		f->private_data = dlb2;
	} else {
		/* SDEVs used for containers */
		int sdev_id = i->i_rdev - dlb2->dev_number - 1;

		if (sdev_id >= 0 && sdev_id < dlb2->num_sdevs)
			f->private_data = dlb2->sdev_dlb2[sdev_id];
		else
			return -EINVAL;
	}
    ...
    printk("DLB BAR start is %llx\n", pci_resource_start(dlb2->pdev, DLB2_FUNC_BAR));
    printk("DLB BAR length is %lld\n", pci_resource_len(dlb2->pdev, DLB2_FUNC_BAR));

    int ret = 0;
    ret = pci_p2pdma_distance(dlb2->pdev, dlb2->dev, true);
    printk("pci_p2pdma_disatance is %d\n", ret);

    if (pci_has_p2pmem(dlb2->pdev)) {
        dev_err(dlb2->dev, "dlb2 has p2p mem\n");
    } else {
        dev_err(dlb2->dev, "dlb2 does not have p2p mem\n");
        dev_err(dlb2->dev, "Allocating p2p mem ...\n");
		ret = pci_p2pdma_add_resource(dlb2->pdev, DLB2_FUNC_BAR, pci_resource_len(dlb2->pdev, DLB2_FUNC_BAR), 0);
		dev_err(dlb2->dev, "dlb2 pci_p2pdma_add_resource returns %d\n", ret);
		pci_p2pmem_publish(dlb2->pdev, true);
		if (pci_has_p2pmem(dlb2->pdev)) {
			dev_err(dlb2->dev, "dlb2 has p2p mem\n");
		} else {
			dev_err(dlb2->dev, "dlb2 does not have p2p mem\n");
    ...
    if (dlb2->hw.p2p_virt_addr == NULL) {
        // dlb2->hw.p2p_virt_addr = pci_alloc_p2pmem(dlb2->pdev, pci_resource_len(dlb2->pdev, DLB2_FUNC_BAR));
        dlb2->hw.p2p_virt_addr = pci_alloc_p2pmem(dlb2->pdev, pci_resource_len(dlb2->pdev, DLB2_FUNC_BAR));
        if (dlb2->hw.p2p_virt_addr == NULL) {
            printk("failed to perform pci_alloc_p2pmem\n");
        } else {
            printk("pci_alloc_p2pmem address is %p\n", dlb2->hw.p2p_virt_addr);
        }
    } else {
        printk("p2pmem has already been allocated at %p\n", dlb2->hw.p2p_virt_addr);
    }
#endif
...
#if P2P_ENABLE
    if (dlb2->hw.p2p_virt_addr != NULL) {
        printk("free p2pmem at %p\n", dlb2->hw.p2p_virt_addr);
        // pci_free_p2pmem(dlb2->pdev, dlb2->hw.p2p_virt_addr, pci_resource_len(dlb2->pdev, DLB2_FUNC_BAR));
        pci_free_p2pmem(dlb2->pdev, dlb2->hw.p2p_virt_addr, pci_resource_len(dlb2->pdev, DLB2_FUNC_BAR));
        dlb2->hw.p2p_virt_addr = NULL;
    } else {
        printk("Cannot free p2pmem at %p\n", dlb2->hw.p2p_virt_addr);
    }
#endif
...
static int dlb2_pp_mmap(struct file *f, struct vm_area_struct *vma)
{
    ...
    // printk("[%s()] Port id: %d\n", __func__, port->id);
    // printk("[%s()] Virtual DLB pp_addr: %lx\n", __func__, vma->vm_start);
    // printk("[%s()] Physical DLB (BAR0) addr: %lx\n", __func__, virt_to_phys(vma->vm_start));
    printk("[%s()] Physical DLB bus addr: %lx\n", __func__, pgoff);
    printk("[%s()] Physical DLB pfn: %lx\n", __func__, pgoff >> PAGE_SHIFT);
		      ...
		      const struct pci_device_id *pdev_id)
{
	...
	/*
	 * Disable pasid so that PF and SRIOV can function properly.
	 * It will be enabled for SIOV mode later.
...
static void dlb2_remove(struct pci_dev *pdev)
{
	...
	if (dlb2->type == DLB2_PF || dlb2->type == DLB2_5_PF) {
		/* Remove all SDEVs for congtainer usage */
		dlb2_sdev_configure(pdev, 0);
