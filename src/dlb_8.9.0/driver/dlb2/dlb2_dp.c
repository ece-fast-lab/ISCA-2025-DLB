	      ...
	      struct dlb2_dp **hdl)
{
	...
	*hdl = dlb2_dp;
    ...
    if (pci_has_p2pmem(dlb2->pdev)) {
        ...
        dev_err(dlb2->dev, "Adding p2p mem ...\n");
		ret = pci_p2pdma_add_resource(dlb2->pdev, DLB2_FUNC_BAR, pci_resource_len(dlb2->pdev, DLB2_FUNC_BAR), 0);
		// ret = pci_p2pdma_add_resource(dlb2->pdev, DLB2_FUNC_BAR, 512*PAGE_SIZE, 0x3000000);
    ...
    if (dlb2->hw.p2p_virt_addr == NULL) {
        ...
        dlb2->hw.p2p_virt_addr = pci_alloc_p2pmem(dlb2->pdev, 1024*PAGE_SIZE);
        if (dlb2->hw.p2p_virt_addr == NULL) {
            printk("failed to perform pci_alloc_p2pmem\n");
        } else {
            printk("pci_alloc_p2pmem address is %lx\n", dlb2->hw.p2p_virt_addr);
        }
    } else {
        printk("p2pmem has already been allocated at %lx\n", dlb2->hw.p2p_virt_addr);
    }
#endif
    // ======
...
#if P2P_ENABLE
    if (dlb2_dp->dlb2->hw.p2p_virt_addr != NULL) {
        printk("free p2pmem at %lx\n", dlb2_dp->dlb2->hw.p2p_virt_addr);
        // pci_free_p2pmem(dlb2_dp->dlb2->pdev, dlb2_dp->dlb2->hw.p2p_virt_addr, pci_resource_len(dlb2_dp->dlb2->pdev, DLB2_FUNC_BAR));
        pci_free_p2pmem(dlb2_dp->dlb2->pdev, dlb2_dp->dlb2->hw.p2p_virt_addr, 1024*PAGE_SIZE);
        dlb2_dp->dlb2->hw.p2p_virt_addr = NULL;
    } else {
        printk("Cannot free p2pmem at %lx\n", dlb2_dp->dlb2->hw.p2p_virt_addr);
    }
	pci_p2pmem_publish(dlb2_dp->dlb2->pdev, false);
#endif
    // ======
...
static int map_producer_port(struct dlb2 *dlb2, struct dlb2_dp_port *port)
{
    ...
    printk("port->pp_addr: %llx\n", (uint64_t)port->pp_addr);
