				      ...
				      void *karg)
{
	...
	if (!ret) {
		/* For sdevs, use host dlb2 for interrupts */
		struct dlb2 *dlb2_intr = dlb2;
		if (dlb2->parent_dlb2)
			dlb2_intr = dlb2->parent_dlb2;
		WRITE_ONCE(dlb2_intr->intr.ldb_cq_intr[arg->port_id].disabled, false);
	}
				      ...
				      void *karg)
{
	...
	if (!ret) {
		...
		if (dlb2->parent_dlb2)
			dlb2_intr = dlb2->parent_dlb2;
		WRITE_ONCE(dlb2_intr->intr.dir_cq_intr[arg->port_id].disabled, false);
	}
				       ...
				       void *karg)
{
	...
	if (!ret) {
		...
		if (dlb2->parent_dlb2)
			dlb2_intr = dlb2->parent_dlb2;
		dlb2_wake_thread(&dlb2_intr->intr.ldb_cq_intr[arg->port_id],
				 WAKE_PORT_DISABLED);
	}
				       ...
				       void *karg)
{
	...
	if (!ret) {
		...
		if (dlb2->parent_dlb2)
			dlb2_intr = dlb2->parent_dlb2;
		dlb2_wake_thread(&dlb2_intr->intr.dir_cq_intr[arg->port_id],
				 WAKE_PORT_DISABLED);
	}
					    ...
					    void *karg)
{
	...
	ret = dlb2_block_on_cq_interrupt(dlb2_intr,
					 domain,
					 arg->port_id,
					 arg->is_ldb,
					 arg->cq_va,
...
#endif
{
	struct dlb2 *dlb2 = f->private_data;
	dlb2_ioctl_fn_t fn;
	void *karg;
	int size;
	int ret;
	...
	if (!dlb2) {
		printk("[%s()] f->private_data is NULL\n", __func__);
		return -EINVAL;
	}
