...
dlb2_pf_map_pci_bar_space(struct dlb2 *dlb2, struct pci_dev *pdev)
{
    ...
    dev_err(dlb2->dev, "[%s()] func_kva: %p, v2p: %llx, func_phys_addr: %lx\n", __func__, dlb2->hw.func_kva, virt_to_phys(dlb2->hw.func_kva), dlb2->hw.func_phys_addr);
    ...
    // p2p_dev = pci_p2pmem_find(dlb2->dev);
    // if (!p2p_dev) {
    //     printk("no peer-to-peer memory is available\n");
    // }

    // int ret = 0;
    // ret = pci_p2pdma_distance(pdev, dlb2->dev, true);
    // printk("pci_p2pdma_disatance is %d\n", ret);

    // ret = pci_p2pdma_add_resource(pdev, DLB2_FUNC_BAR, PAGE_SIZE, 0x2300000);
    // dev_err(dlb2->dev, "dlb2 pci_p2pdma_add_resource returns %d\n", ret);
    // pci_p2pmem_publish(pdev, true);

    // struct pci_dev *nic_dev;
    // nic_dev = pci_get_device(0x8086, 0x1521, NULL);
    // if (!nic_dev)
    // {
    ...
    // clients[0] = nic_dev->dev)
    // pci_p2pdma_distance_many(pdev, &clients, 1, true);

    // if (pci_has_p2pmem(pdev)) {
    //     dev_err(dlb2->dev, "dlb2 has p2p mem\n");
    // } else {
    //     dev_err(dlb2->dev, "dlb2 does not have p2p mem\n");
    // }

    // dev_err(dlb2->dev, "[%s()] func_kva: %p, func_phys_addr: %lx, %p\n", __func__, dlb2->hw.func_kva, dlb2->hw.func_phys_addr, (void *)(dlb2->hw.func_phys_addr));
    // dev_err(dlb2->dev, "[%s()] csr_kva: %p, csr_phys_addr: %lx, %p\n", __func__, dlb2->hw.csr_kva, dlb2->hw.csr_phys_addr, (void *)(dlb2->hw.csr_phys_addr));


	// void __iomem *io_addr;
	// io_addr = ioremap(dlb2->hw.func_phys_addr, PAGE_SIZE);
    // dev_err(dlb2->dev, "[%s()] func_kva: %p, func_phys_addr: %lx, virtual addr: %p, virt_to_phy addr: %llx\n", __func__, dlb2->hw.func_kva, dlb2->hw.func_phys_addr, io_addr, virt_to_phys(io_addr));
    // iounmap(io_addr);
    // void __iomem *io_addr_incr;
    // io_addr_incr = io_addr;
	// if (io_addr_incr != NULL) {
	// 	for (int i = 0; i < 8; i+=4) {
    //         unsigned int ret = ioread32(io_addr_incr);
    //         rmb();
    //         dev_err(dlb2->dev, "[%s()] csr addr: %p has ioremap data: %x\n", __func__, io_addr_incr, ret);
    //         io_addr_incr+=4;
    //     }
    //     iounmap(io_addr);
	// }
    
	// unsigned long func_kva_flag = pci_resource_flags(pdev, DLB2_FUNC_BAR);
	// unsigned long csr_kva_flag = pci_resource_flags(pdev, DLB2_CSR_BAR);
	// // dev_err(dlb2->dev, "[%s()] func_kva_flag: %lx, csr_kva_flag: %lx\n", __func__, func_kva_flag, csr_kva_flag);
	// printk("[%s()] func_kva_flag: %lx ========\n", __func__, func_kva_flag);
	// if (func_kva_flag & IORESOURCE_MEM) { printk(KERN_INFO "  Memory-mapped I/O region\n"); }
    // if (func_kva_flag & IORESOURCE_IO) { printk(KERN_INFO "  I/O port region\n"); }
    // if (func_kva_flag & IORESOURCE_PREFETCH) { printk(KERN_INFO "  Prefetchable region\n"); }
    // if (func_kva_flag & IORESOURCE_READONLY) { printk(KERN_INFO "  Read-only region\n"); }
    // if (func_kva_flag & IORESOURCE_CACHEABLE) { printk(KERN_INFO "  Cacheable region\n"); }
    // if (func_kva_flag & IORESOURCE_RANGELENGTH) { printk(KERN_INFO "  Length of the resource region\n"); }
    // if (func_kva_flag & IORESOURCE_SHADOWABLE) { printk(KERN_INFO "  Shadowable ROM region\n"); }
    // if (func_kva_flag & IORESOURCE_SIZEALIGN) { printk(KERN_INFO "  Size-aligned region\n"); }
    // if (func_kva_flag & IORESOURCE_STARTALIGN) { printk(KERN_INFO "  Start-aligned region\n"); }
    // if (func_kva_flag & IORESOURCE_DISABLED) { printk(KERN_INFO "  Disabled region\n"); }
    // if (func_kva_flag & IORESOURCE_UNSET) { printk(KERN_INFO "  Region unset\n"); }
    // if (func_kva_flag & IORESOURCE_AUTO) { printk(KERN_INFO "  Auto-configured region\n"); }
    // if (func_kva_flag & IORESOURCE_BUSY) { printk(KERN_INFO "  Busy region\n"); }

	// printk("[%s()] csr_kva_flag: %lx ========\n", __func__, csr_kva_flag);
	// if (csr_kva_flag & IORESOURCE_MEM) { printk(KERN_INFO "  Memory-mapped I/O region\n"); }
    // if (csr_kva_flag & IORESOURCE_IO) { printk(KERN_INFO "  I/O port region\n"); }
    // if (csr_kva_flag & IORESOURCE_PREFETCH) { printk(KERN_INFO "  Prefetchable region\n"); }
    // if (csr_kva_flag & IORESOURCE_READONLY) { printk(KERN_INFO "  Read-only region\n"); }
    // if (csr_kva_flag & IORESOURCE_CACHEABLE) { printk(KERN_INFO "  Cacheable region\n"); }
    // if (csr_kva_flag & IORESOURCE_RANGELENGTH) { printk(KERN_INFO "  Length of the resource region\n"); }
    // if (csr_kva_flag & IORESOURCE_SHADOWABLE) { printk(KERN_INFO "  Shadowable ROM region\n"); }
    // if (csr_kva_flag & IORESOURCE_SIZEALIGN) { printk(KERN_INFO "  Size-aligned region\n"); }
    // if (csr_kva_flag & IORESOURCE_STARTALIGN) { printk(KERN_INFO "  Start-aligned region\n"); }
    // if (csr_kva_flag & IORESOURCE_DISABLED) { printk(KERN_INFO "  Disabled region\n"); }
    // if (csr_kva_flag & IORESOURCE_UNSET) { printk(KERN_INFO "  Region unset\n"); }
    // if (csr_kva_flag & IORESOURCE_AUTO) { printk(KERN_INFO "  Auto-configured region\n"); }
    // if (csr_kva_flag & IORESOURCE_BUSY) { printk(KERN_INFO "  Busy region\n"); }
...
#define DLB2_SDEV_SYSFS_ATTRS(id) \
static struct attribute *dlb2_sdev ## id ## _attrs[] = {\
	&dev_attr_vf ## id ## _sched_domains.attr,	\
	&dev_attr_vf ## id ## _ldb_queues.attr,		\
	&dev_attr_vf ## id ## _ldb_ports.attr,		\
	&dev_attr_vf ## id ## _cos0_ldb_ports.attr,	\
...
#define DLB2_SDEV_SYSFS_ATTR_GROUP(id) \
DLB2_SDEV_SYSFS_ATTRS(id);					\
static struct attribute_group dlb2_sdev ## id ## _attr_group = {\
	.attrs = dlb2_sdev ## id ## _attrs,			\
	.name = "sdev" #id "_resources"				\
}
...
DLB2_SDEV_SYSFS_ATTR_GROUP(0);
DLB2_SDEV_SYSFS_ATTR_GROUP(1);
DLB2_SDEV_SYSFS_ATTR_GROUP(2);
DLB2_SDEV_SYSFS_ATTR_GROUP(3);
DLB2_SDEV_SYSFS_ATTR_GROUP(4);
DLB2_SDEV_SYSFS_ATTR_GROUP(5);
DLB2_SDEV_SYSFS_ATTR_GROUP(6);
DLB2_SDEV_SYSFS_ATTR_GROUP(7);
DLB2_SDEV_SYSFS_ATTR_GROUP(8);
DLB2_SDEV_SYSFS_ATTR_GROUP(9);
DLB2_SDEV_SYSFS_ATTR_GROUP(10);
DLB2_SDEV_SYSFS_ATTR_GROUP(11);
DLB2_SDEV_SYSFS_ATTR_GROUP(12);
DLB2_SDEV_SYSFS_ATTR_GROUP(13);
DLB2_SDEV_SYSFS_ATTR_GROUP(14);
DLB2_SDEV_SYSFS_ATTR_GROUP(15);

const struct attribute_group *dlb2_sdev_attrs[] = {
	&dlb2_sdev0_attr_group,
	&dlb2_sdev1_attr_group,
	&dlb2_sdev2_attr_group,
	&dlb2_sdev3_attr_group,
	&dlb2_sdev4_attr_group,
	&dlb2_sdev5_attr_group,
	&dlb2_sdev6_attr_group,
	&dlb2_sdev7_attr_group,
	&dlb2_sdev8_attr_group,
	&dlb2_sdev9_attr_group,
	&dlb2_sdev10_attr_group,
	&dlb2_sdev11_attr_group,
	&dlb2_sdev12_attr_group,
	&dlb2_sdev13_attr_group,
	&dlb2_sdev14_attr_group,
	&dlb2_sdev15_attr_group,
};
...
static ssize_t num_sdevs_show(struct device *dev,
			   struct device_attribute *attr,
			   char *buf)
{
	struct dlb2 *dlb2 = dev_get_drvdata(dev);
...
static ssize_t num_sdevs_store(
	struct device *dev,
	struct device_attribute *attr,
	const char *buf,
	size_t count)
{
	...
	err = dlb2_sdev_configure(dlb2->pdev, val);
