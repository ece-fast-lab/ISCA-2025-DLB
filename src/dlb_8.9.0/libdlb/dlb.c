 ...
 */
#define DLB_ZERO_DEQ_CREDIT_RETURN_THRES 64
 ...
 */
#define DLB_BATCH_SZ_EXCEED_CREDIT_RETURN_THRES 64
...
dlb_domain_hdl_t dlb_attach_sched_domain(
    dlb_hdl_t hdl,
    int domain_id,
    dlb_shared_domain_ptr_t **shared_domain_ptr)
{
    dlb_domain_hdl_internal_t *domain_hdl = NULL;
    dlb_shared_domain_t *shared_domain = NULL;
    dlb_local_domain_t *local_domain;
    ...
    printf("shared_domain is %p %p %p\n", shared_domain, &shared_domain->sw_credits.avail_credits[LDB], &shared_domain->sw_credits.avail_credits[DIR]);
    if (shared_domain_ptr != NULL) {
        // *shared_domain_ptr = shared_domain;
        memcpy(shared_domain_ptr, &(shared_domain), sizeof(void *));
    }
    ...
    dlb_port_hdl_internal_t *port_hdl)
{
    ...
    // printf("[%s()] Have DLB pp_addr: %p\n", __func__, port_hdl->pp_addr);
    ...
    uint64_t **dlb_pp_addr)
{
    ...
    // dlb_pp_addr = &(port_hdl->pp_addr);
    if (dlb_pp_addr != NULL) {
        memcpy(dlb_pp_addr, &(port_hdl->pp_addr), sizeof(void *));
        // printf("[%s()] Try to get DLB pp_addr: %p\n", __func__, port_hdl->pp_addr);
        // printf("[%s()] Try to get DLB shared_base: %p\n", __func__, port_hdl->shared_base);
    }
    // ========

    if (movdir64b_supported()) {
        port_hdl->enqueue_four = dlb_movdir64b;
        printf("Support dlb_movdir64b\n");
    } else {
        port_hdl->enqueue_four = dlb_movntdq;
        printf("Do not support dlb_movdir64b, use dlb_movntdq\n");
    }
    ...
    uint64_t **dlb_pp_addr)
{
    ...
    port_hdl->credit_pool[LDB] = &ldb_pool->avail_credits;
    port_hdl->credit_pool[DIR] = &dir_pool->avail_credits;
    printf("credit pool: %u %u\n", *port_hdl->credit_pool[LDB], *port_hdl->credit_pool[DIR]);
    ...
    for (port_type = 0; port_type < NUM_PORT_TYPES; ++port_type) {
        ...
        if (cond && port_hdl->port.shared->credits[port_type].num > credit_threshold) {
            if (port_hdl->port.shared->credit_return_count[type] >= cnt_threshold) {
                // uint32_t credits = *port_hdl->credit_pool[port_type];
                ...
                // printf("dlb2_check_and_release_credits: check type %d port type %d port %d: release %d, credit pool has %d at %p\n", type, port_type, port_hdl->port.shared->id, val, __sync_fetch_and_add(port_hdl->credit_pool[port_type], 0), port_hdl->credit_pool[port_type]);

                // if (credits && __sync_bool_compare_and_swap(port_hdl->credit_pool[port_type], credits, credits - val)) {        
                //     port_hdl->port.shared->credits[port_type].num -= val;
                //     // printf("credit_threshold is %d\n", credit_threshold);	
                //     printf("dlb2_check_and_release_credits: check type %d port type %d port %d: release %d, credit pool has %d at %p\n", type, port_type, port_hdl->port.shared->id, val, __sync_fetch_and_add(port_hdl->credit_pool[port_type], 0), port_hdl->credit_pool[port_type]);
                // }
            } else {
                cnt_reset = false;
    ...
    dlb_port_type_t type)
{
    ...
    // printf("port %d: credit pool has %d, batch_size is %d, new credit pool is %d\n", port->id, credits, batch_size, *port_hdl->credit_pool[type]);
}
    ...
    bool is_2_5)
{
    ...
    // printf("port %d: dlb2_inc_port_credits returns %d credits on port type %d, total: %d\n", port->id, num, port->type, port->credits[port->type].num);
    ...
    bool is_2_5)
{
    ...
    for (i = 0; i < num; i += 4) {
        ...
        if (j != 0) {
            // printf("[__dlb2_adv_send_no_credits] element sent:\n");
            // for (int a = 0; a < 4*sizeof(dlb_enqueue_qe_t); a++) {
            //     printf("%x ", ((char *)(enqueue_qe))[a]);
            //     if ((a+1) % sizeof(dlb_enqueue_qe_t) == 0)
            //         printf("\n");
            // }
            // for (int b = 0; b < 4; b++) {
            //     printf("size of dlb_enqueue_qe_t is %d\n", sizeof(dlb_enqueue_qe_t));
            //     printf("data: %lu\n", enqueue_qe[b].data);
            //     printf("opaque: %d\n", enqueue_qe[b].opaque);
            //     printf("qid: %d\n", enqueue_qe[b].qid);
            //     printf("sched_byte: %d\n", enqueue_qe[b].sched_byte);
            //     printf("flow_id: %u\n", enqueue_qe[b].flow_id);
            //     printf("num_tokens_minus_one: %d\n", enqueue_qe[b].num_tokens_minus_one);
            //     printf("meas_lat: %d\n", enqueue_qe[b].meas_lat);
            //     printf("weight: %d\n", enqueue_qe[b].weight);
            //     printf("no_dec: %d\n", enqueue_qe[b].no_dec);
            //     printf("cmp_id: %d\n", enqueue_qe[b].cmp_id);
            //     printf("misc_byte: %d\n", enqueue_qe[b].misc_byte);
            //     printf("cmd_byte: %d\n", enqueue_qe[b].cmd_byte);
            // }
            port_hdl->enqueue_four(enqueue_qe, port_hdl->pp_addr);
            // printf("[%s()] Access DLB pp_addr when enqueue: %p\n", __func__, port_hdl->pp_addr);
            // printf("[%s()] Enqueue value is: %lX, %lX\n", __func__, enqueue_qe[0].data, *(port_hdl->pp_addr));
        }
    ...
    bool credits_required_for_all_cmds)
{
    ...
    for (i = 0; i < num; i += 4) {
        ...
        if (j != 0) {
            // printf("[__dlb2_adv_send] element sent:\n");
            // for (int a = 0; a < 4*sizeof(dlb_enqueue_qe_t); a++) {
            //     printf("%x ", ((char *)(enqueue_qe))[a]);
            //     if ((a+1) % sizeof(dlb_enqueue_qe_t) == 0)
            //         printf("\n");
            ...
            port_hdl->enqueue_four(enqueue_qe, port_hdl->pp_addr);
            // printf("[%s()] Access DLB pp_addr when enqueue: %p\n", __func__, port_hdl->pp_addr);
            // _movdir64b(port_hdl->pp_addr, enqueue_qe);
            // _mm_mfence();
            // dlb_enqueue_qe_t *qe_from_dlb;
            // qe_from_dlb = (dlb_enqueue_qe_t *)calloc(4, sizeof(dlb_enqueue_qe_t));
            // if (qe_from_dlb != NULL) {
            //     // memset(qe_from_dlb, 0, sizeof(dlb_enqueue_qe_t)*4);
            //     printf("here\n");
            //     // _mm_mfence();
            //     asm volatile(".byte 0x66, 0x0f, 0x38, 0xf8, 0x02"
            //         :
            //         : "a" (qe_from_dlb), "d" (port_hdl->pp_addr));
            //     // _mm_mfence();
            // }
            // free(qe_from_dlb);
            // printf("here1\n");
            // _mm_sfence();
            // volatile uint64_t * read_pp_addr = port_hdl->pp_addr;
            // volatile uint64_t temp_data = 0;
            // temp_data = ((volatile dlb_enqueue_qe_t *)read_pp_addr)[0].data;
            // printf("[%s()] Access DLB pp_addr when enqueue: %p. Enqueue value is: %lX, %lX\n", __func__, port_hdl->pp_addr, enqueue_qe[0].data, temp_data);
        }
    ...
    // printf("port %d: port->credits[LDB].num is %d, used_credits[LDB] is %d\n", port->id, port->credits[LDB].num, used_credits[LDB]);
    // printf("port %d: port->credits[DIR].num is %d, used_credits[DIR] is %d\n", port->id, port->credits[DIR].num, used_credits[DIR]);
    ...
    dlb_recv_t *event)
{
    ...
    if (wait && PORT_CQ_IS_EMPTY(port_hdl, port)) {
        ...
        ASSERT((port->enabled || !PORT_CQ_IS_EMPTY(port_hdl, port)), EACCES);
        // printf("wait && PORT_CQ_IS_EMPTY(port_hdl, port)\n");
    }
