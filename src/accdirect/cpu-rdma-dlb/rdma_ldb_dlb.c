...
static void *rdma_rx_thread(void *__args)
{
    thread_args_t *args = (thread_args_t *) __args;
    dlb_event_t events[NUM_EVENTS_PER_LOOP];
    int64_t num_loops, i, num_tx = 0;
    ...
    int tid = args->tid;
    int meta_conn_id = args->meta_conn_id;
    int dlb_conn_id = args->dlb_conn_id;

    struct dev_context dev_ctx = args->dev_ctx;
    struct conn_context *conn_ctxs = args->conn_ctxs;

    num_loops = (num_events == 0) ? -1 : num_events / NUM_EVENTS_PER_LOOP;

    for (i = 0; i < NUM_EVENTS_PER_LOOP; i++) {
        events[i].adv_send.queue_id = 31 - tid % num_queues;
    ...
    while (!worker_done) {
            ...
            if (metadata_offset + 4 >= meta_size && metadata_offset + 1 < meta_size) {
                ...
                for (int j = valid_qe; j < (meta_size-metadata_offset); j++) {
                    if (((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset+j].valid != 0) {
                        events[j].adv_send.udata64 = ((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset+j].addr;
                        // printf("Payload addr is %lx\n", events[j].adv_send.udata64);
                        events[j].adv_send.udata16 = ((struct rdma_metadata *)(conn_ctxs[meta_conn_id].data_buf))[metadata_offset+j].wr_id;
        ...
        ret = 0;
        num = 0;
        for (int j = 0; num != valid_qe && !worker_done; j++) {
            ret = dlb_send(args->port, valid_qe-num, &events[num]);
            if (ret == -1)
                printf("dlb_send failed\n");
            num += ret;
        }
        ...
        // printf("ecnt is %d\n", ecnt);
        if (metadata_offset == meta_size && meta_conn_id != num_client_conns-1) {
            // printf("ecnt is %d\n", ecnt);
            send_ack = 0;
...
static void *rx_traffic(void *__args)
{
    ...
    for (i = 0; num_rx < num_events && num_loops != -1; i++)
    {
        ...
        // printf("\n");
        // for (int a = 0; a < 4; a++) {
        //     printf("dequeue element data is %lx:\n", events[a].recv.udata64);
        //     printf("dequeue element data is %lx:\n", events[a].recv.udata16);
        //     printf("Payload is %s\n", (char *)events[a].recv.udata64);
        //     // for (int b = 0; b < sizeof(dlb_dequeue_qe_t); b++) {
        //     //     printf("%x ", ((char *)(&(events[a].recv)))[b]);
        //     // }
        //     printf("\n");
        // } 
    ...
    for (i = 0; !worker_done; i++) {
        ...
        for (j = 0; j < num_rx; j++) {
            ...
            if (remote_tid == num_client_conns - 1) {
                post_send(&(conn_ctxs[num_client_conns + num_dlb_pp - 1]), bufs_num*PAGE_SIZE, 16);
                total_latency_test++;
                wait_completions(&(conn_ctxs[num_client_conns + num_dlb_pp - 1]), SEND_WRID, 1);
            }
        }
        worker_pkt_count[tid] = total_latency_test;
...
{
    pthread_t rx_thread, *tx_threads, *worker_threads, monitor_thread;
    ...
    tx_args = (thread_args_t *)calloc(num_txs, sizeof(thread_args_t));
    if (!tx_args) {
        free(tx_threads);
        return -ENOMEM;
    }
    ...
    for (int i = 0; i < num_txs; i++) {
        ...
        tx_args[i].port = tx_ports[i];
        tx_args[i].core_mask = core_idx;
        core_idx += 1;
        tx_args[i].tid = i;
        tx_args[i].dlb_conn_id = num_dlb_pp + i;
        tx_args[i].meta_conn_id = i;
    ...
    int num_qps = 2 * num_client_conns;
    ...
    int snic_sockfd = -1;
    if (host_type == CLIENT) {
        printf("Connecting server for metadata and data transter %s ...\n", server_ip);
    ...
    /* Launch threads */
    for (i = 0; i < num_txs; i++) {
        tx_args[i].dev_ctx = dev_ctx;
        tx_args[i].conn_ctxs = conn_ctxs;
        pthread_create(&tx_threads[i], NULL, rdma_rx_thread, &tx_args[i]);
    }
    for (i = 0; i < num_workers; i++) {
