...
static void *latency_client_thread(void *__args)
{
    ...
    for (int i = 0; i < num_events; i++) {
        ...
        int ne = 0;
        while (ne < 2) {
            ne += ibv_poll_cq(conn_ctxs[data_conn_id].cq, 1, &wc[ne]);
            if (ne < 0) {
        ...
        for (int k = 0; k < ne; k++) {
            if (wc[k].wr_id == RECV_WRID) {
                ...
                post_recv(&conn_ctxs[data_conn_id], bufs_num*PAGE_SIZE+(i%128)*16, 16);
            }
            if (wc[k].status != IBV_WC_SUCCESS) {
                fprintf(stderr, "Failed status %s (%d) for wr_id %d\n", ibv_wc_status_str(wc[k].status), wc[k].status, wc[k].wr_id);
            }
        }
    }
...
static void *client_thread(void *__args)
{
    ...
    for (int i = 0; test_finished == 0; i++) {      
        ...
        for (int j = 0; j < BATCH_SIZE-1; j++) {
            ...
            prepare_send_write_batch(&conn_ctxs[data_conn_id], cnt*msg_size, cnt*msg_size, msg_size, 0);
        }
        cnt++;
        ((uint64_t *)(conn_ctxs[data_conn_id].data_buf+(cnt)*msg_size))[0] = 0;

        if (cnt % BATCH_SIZE == 0 && cnt < meta_size) {
            prepare_send_write_batch(&conn_ctxs[data_conn_id], cnt*msg_size, cnt*msg_size, msg_size, 0);
            post_send_write_batch(&conn_ctxs[data_conn_id]);
        }
        else if (cnt == meta_size) {
            prepare_send_write_batch(&conn_ctxs[data_conn_id], cnt*msg_size, cnt*msg_size, msg_size, 1);
            post_send_write_batch(&conn_ctxs[data_conn_id]);
        ...
        for (int j = 0; j < BATCH_SIZE; j++) {      
            ((struct rdma_metadata *)(conn_ctxs[tid].data_buf))[metadata_offset+j%4].addr = conn_ctxs[data_conn_id].remote_mr.addr + remote_offset;
            remote_offset = (remote_offset + msg_size) % (bufs_num*PAGE_SIZE);
        ...
        if (i != 0 && metadata_offset == meta_size-BATCH_SIZE) {
...
int main (int argc, char *argv[]) {
    ...
    num_qps = num_meta_conns + num_client_conns;
    ...
    for (int i = 0; i < num_meta_conns - 1; i++) {
        ...
        worker_args[i].data_conn_id = num_meta_conns + i;
    ...
    worker_args[num_meta_conns - 1].data_conn_id = num_meta_conns + num_client_conns - 1;
